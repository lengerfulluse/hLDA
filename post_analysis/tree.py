...
def write_topic_tree_dot(state,
    ...
    outfile.write("node [shape=plaintext, fontname=Helvetica];\n")
    outfile.write("edge [style=solid, arrowhead=empty, arrowsize=1];\n")
...
#    outfile.write('params [shape=rectangle, style=bold, color=red,fontcolor=red, fontsize=10, label="ETA = %s\\nGAMMA = %s\\nGEM MEAN = %s\\nGEM SCALE=%s\\nSCORE = %s"]\n' %
#                  (eta, gamma, gem_mean, gem_scale,score))

    skip = {}
    fontsizes = [18, 14, 12, 10]
...
def main(type,
    ...
    if (type == 'txt'):
        ...
        if os.path. isfile(dmap_filename):
            docmap = read_docmap(dmap_filename)
        write_topic_tree_dot(state, docmap, out_filename)
